...
function focusHandlers(error) {
  return {
    ...
    onBlur: e => {
      e.target.style.borderColor = error ? color.danger : color.border;
...
export function Label({ children, htmlFor, required, hint, style }) {
  return (
    <label htmlFor={htmlFor} style={{
      ...
      marginBottom: 6, fontSize: font.size.md, fontWeight: font.weight.semibold, color: color.text, ...style,
    }}>
      <span>
        {children}
        {required && <span style={{ color: color.danger, marginLeft: 3 }}>*</span>}
      </span>
      {hint && <span style={{ fontSize: font.size.sm, color: color.textSubtle, fontWeight: font.weight.regular }}>{hint}</span>}
    </label>
  );
}
...
export function Input({ error, icon, style, ...props }) {
  if (icon) {
    return (
      <div style={{ position: 'relative', width: '100%' }}>
        <span style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: color.textFaint, display: 'inline-flex' }}>{icon}</span>
        <input
          {...props}
          style={{
            ...baseField,
  ...
  return (
    <input
      {...props}
      style={{ ...baseField, borderColor: error ? color.danger : color.border, ...style }}
      {...focusHandlers(error)}
    />
  );
}
...
export function Textarea({ error, style, rows = 4, ...props }) {
  return (
    <textarea
      rows={rows}
      {...props}
      style={{ ...baseField, resize: 'vertical', minHeight: 90, borderColor: error ? color.danger : color.border, ...style }}
      {...focusHandlers(error)}
    />
  );
}
    ...
    <div style={{ position: 'relative', width: '100%' }}>
      <select
        ...
        style={{
          ...
          borderColor: error ? color.danger : color.border, ...style,
        }}
        {...focusHandlers(error)}
      >
        {children}
      </select>
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={color.textSubtle} strokeWidth="2"
  ...
  return (
    <label htmlFor={id} style={{
      ...
      <span style={{
        ...
        background: checked ? color.brand500 : '#cbd5e1',
      ...
      {(label || description) && (
        <span style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
          {label && <span style={{ fontSize: font.size.base, fontWeight: font.weight.semibold, color: color.text }}>{label}</span>}
          {description && <span style={{ fontSize: font.size.md, color: color.textSubtle, lineHeight: 1.4 }}>{description}</span>}
        </span>
      )}
    </label>
  );
...
export function FormSection({ title, description, children, style }) {
  return (
    <section style={{
      padding: '24px 28px', borderBottom: `1px solid ${color.divider}`, ...style,
    }}>
      {(title || description) && (
        <header style={{ marginBottom: 20, display: 'grid', gridTemplateColumns: '280px 1fr', gap: 32, alignItems: 'start' }}>
          <div>
            {title && <h3 style={{ margin: 0, fontSize: font.size.lg, fontWeight: font.weight.bold, color: color.text, letterSpacing: '-0.01em' }}>{title}</h3>}
            {description && <p style={{ margin: '6px 0 0', fontSize: font.size.md, color: color.textSubtle, lineHeight: 1.5 }}>{description}</p>}
          </div>
